...
`.github/workflows/release.yml` handles tagged releases: pushing a tag matching `v*.*.*` (e.g. `v1.0.0`) triggers a workflow that runs the test suite, strips the leading `v` from the tag, publishes with that exact version as a one-off `-p:Version=X.Y.Z` override (a build-time argument, not a file CI writes back to the repo), and uploads the resulting `normalize-text.exe` (bare filename, no version/OS suffix — matching every other Strange project's release convention, so replacing an installed copy on `PATH` is always a straight overwrite, never a rename) to a GitHub Release created from that tag via `gh release create --generate-notes`, titled `NormalizeText X.Y.Z` — the tag itself stays bare (`vX.Y.Z`, no product name), only the release's display title gets the product-name prefix, matching the convention used across other Strange projects. A release build's version therefore always matches its tag exactly, with no `+<git-sha>` suffix (the working tree at a tag *is* that exact commit, so the suffix would be redundant) — that suffix only ever appears on a plain local build/clone, which is exactly the case it exists to identify.
